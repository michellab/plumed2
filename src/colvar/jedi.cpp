...
jedi::jedi(const ActionOptions &ao) : PLUMED_COLVAR_INIT(ao),
                                      pbc(true),
                                      benchmark(false),
...
#pragma omp parallel for num_threads(nthreads)
  for (unsigned j = 0; j < n_apolarpolar; j++)
  {
    ...
    for (unsigned i = 0; i < size_grid; i++)
    {
      ...
      double d_Sonexposurei_xpj = d_Sonexposurei_m * (1.0 / params.deltaE) * d_exposure_xpj;
      //cout << "  d_exposure_xpj" <<  d_exposure_xpj << endl;
      double d_Sonexposurei_ypj = d_Sonexposurei_m * (1.0 / params.deltaE) * d_exposure_ypj;
      double d_Sonexposurei_zpj = d_Sonexposurei_m * (1.0 / params.deltaE) * d_exposure_zpj;
      // d_ai_xpj
    ...
    for (unsigned i = 0; i < size_grid; i++)
    {
      ...
      double deriv_x = d_Soffrij_m * (1 / params.r_hydro) * d_rij_xpj + d_Soffrij_ai * d_ai_xpj_vec[i];
      double deriv_y = d_Soffrij_m * (1 / params.r_hydro) * d_rij_ypj + d_Soffrij_ai * d_ai_ypj_vec[i];
      double deriv_z = d_Soffrij_m * (1 / params.r_hydro) * d_rij_zpj + d_Soffrij_ai * d_ai_zpj_vec[i];
  ...
  vector<double> d_jedi_dx_nocorrection;
  vector<double> d_jedi_dy_nocorrection;
  vector<double> d_jedi_dz_nocorrection;
  for (unsigned j = 0; j < n_apolarpolar; j++)
  {
    d_jedi_dx_nocorrection.push_back(d_Jedi_xpj_vec[j]);
    d_jedi_dy_nocorrection.push_back(d_Jedi_ypj_vec[j]);
    d_jedi_dz_nocorrection.push_back(d_Jedi_zpj_vec[j]);
  }
  ...
  */
  ...
  if (!iszero)
  {
    ...
    for (unsigned i = 0; i < size_grid; i++)
    {
      ...
      wfile << std::fixed << i << " " << std::setprecision(5) << ai << " " << std::setprecision(5) << lig_i << " " << std::setprecision(5) << cc_i << " " << std::setprecision(5) << s_on_exposure_i << " " << endl;
    }
    wfile.close();
  }
